...
void wdw_volume_tri_origin(
  ...
  dw[0] = (node2xyz[1].cross(node2xyz[2])) * (1.f / 6.f);
  dw[1] = (node2xyz[2].cross(node2xyz[0])) * (1.f / 6.f);
  dw[2] = (node2xyz[0].cross(node2xyz[1])) * (1.f / 6.f);
}
    ...
    for (unsigned int inode = 0; inode < 3; ++inode) {
      for (unsigned int idim = 0; idim < 3; ++idim) {
        // write some code including `dw` and `lambda`
          dW(3 * node2vtx[inode] + idim) -= lambda * dw[inode](idim);
          ddW(3 * num_vtx, 3 * node2vtx[inode] + idim) -= dw[inode](idim);
          ddW(3 * node2vtx[inode] + idim, 3 * num_vtx) -= dw[inode](idim);
      }
    }
  }
  // Do not forget to write one line of code here
  dW(3 * num_vtx) -= (volume - volume_trg);
  // Do not change below
